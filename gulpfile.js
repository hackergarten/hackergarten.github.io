...
const LOCATION_RESOLVER_BASE_URL = 'https://nominatim.openstreetmap.org';

https: var hashCode = function (str) {
    var hash = 0;
...
gulp.task(
    'generate-xml',
    gulp.series('validate-events', function () {
        return gulp
            .src('./events.json')
            .pipe(jsonlint())
            .pipe(jsonlint.failOnError())
                    ...
                    for (var i = 0; i < events.length; i++) {
                        ...
                        if (new Date(event.date) >= today) {
                            ...
                            if (event.title && event.location) {
                                // old JSON format without venue and address
                                event.title =
                                    'on ' +
                                    event.date +
                                    ' at ' +
                                    ...
                                    ' in ' +
                                    event.location;
                            } else if (event.venue && event.address) {
                                // new JSON format with venue and address
                                event.title =
                                    'on ' +
                                    event.date +
                                    ' at ' +
                                ...
                                // fallback with title and date
                                event.title += ' on ' + event.date;
                            }
                            xml.push('<item>');
                            xml.push(
                                '<title>' +
                                    escapeHTML('Hackergarten ' + event.title) +
                                    '</title>'
                            );
                            xml.push(
                                '<link>https://hackergarten.net/#event-' +
                                    hash +
                                    '</link>'
                            );
...
gulp.task(
    ...
    gulp.series('validate-events', function () {
        return gulp
            ...
            .pipe(jsonlint.reporter())
            .pipe(
                transform(async function (contents) {
                    ...
                    for (var i = 0; i < recentEvents.length; i++) {
                        ...
                        uniqueLocations[recentEvents[i].address].push(
                            recentEvents[i]
                        );
                    }
                    var locations = [];
                    for (var address in uniqueLocations) {
                        const event = uniqueLocations[address][0];

                        const { lat, long } = await searchLocation(
                            event.address
                        );

                        locations.push({
                            title: 'Hackergarten City',
                            sponsors: [event.venue],
                            link: getLink(event),
                            lat,
                            long,
                        });
...
async function searchLocation(query) {
    const url = `${LOCATION_RESOLVER_BASE_URL}/search?q=${encodeURIComponent(
        query
    ...
    try {
        ...
        if (!response.ok) {
            console.log(
                "An error occured, but I don't know why",
                await response.text()
            );
        }

        const [first, ...data] = await response.json();
        ...
        return gulp
            ...
            .pipe(
                transform(function (contents) {
                    ...
                    html.push('<body>');
                    html.push(
                        '<table class="table table-striped"><tr><th>#</th><th>Project</th><th>Count</th></tr>'
                    );
...
gulp.task(
    'default',
    gulp.series('generate-xml', 'generate-projects', 'generate-locations')
);
